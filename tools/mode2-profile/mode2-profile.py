...
Subcommands:
  derive  <vbmeta.img> -o <profile.toml>  — derive a profile TOML from a
                                            stock vbmeta image.
  compile <profile.toml> -o <profile.bin> — compile a profile TOML to the
...
def cmd_compile(args) -> int:
    ...
    try:
        with toml_path.open("rb") as _fh:
            prof = tomllib.load(_fh)
...
def main() -> int:
    ...
    d = sub.add_parser("derive", help="derive a profile TOML from a vbmeta image")
    d.add_argument("vbmeta", help="stock vbmeta.img")
    d.add_argument("-o", "--output", required=True, help="output profile TOML path")
    d.set_defaults(func=cmd_derive)

    c = sub.add_parser("compile", help="compile a profile TOML to a 120-byte binary")
    c.add_argument("toml", help="gbl-chainload_profile.toml")
